...
@objective(model, Min, sum(z_reached[i] * uniform_time_step[i] for i in 1:(k-1)))
...
@constraint(model, [i=1:k], x[i] .- final_x <= M * (z_reached[i]))
@constraint(model, [i=1:k], x[i] .- final_x >= - M * (z_reached[i]))
@constraint(model, [i=1:k], y[i] .- final_y <= M * (z_reached[i]))
@constraint(model, [i=1:k], y[i] .- final_y >= - M * (z_reached[i]))
...
plot(obstacle_polygon_x_coordinates, obstacle_polygon_y_coordinates,seriestype=:shape, aspect_ratio=:equal)
