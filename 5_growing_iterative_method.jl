include("helpers.jl")
function solveIterativeVehicleMilp(_initial_state::StateVector, _final_state::StateVector, _discretization_steps, _obstacle, _time_intervals)
...
_original_obstacle = CircularObstacle(4,4, obstacle_radius)
ori_obstacle_polygon_x_coordinates, ori_obstacle_polygon_y_coordinates = generatePolygon(8, _obstacle.x , _obstacle.y, _original_obstacle.radius)
...
solution, optimal_solution = solveIterativeVehicleMilp(_initial_state, _final_state, _discretization_steps, _obstacle, _time_intervals)
...
plot!(ori_obstacle_polygon_x_coordinates, ori_obstacle_polygon_y_coordinates, seriestype=:shape, aspect_ratio=:equal, color=:orange)
...
_obstacle.radius *= obstacle_scale
...
while length(_collision_indexes) != 0
    ...
    _collision_indexes = checkCollision(solution, _original_obstacle)
    _time_intervals = computeTimeIntervals(_collision_indexes,_time_intervals)
    _discretization_steps = length(_time_intervals)
    _obstacle.radius *= obstacle_scale
